...
'''
def searchLinkedlist(L,element):
    retorno = None
    cont = 0
    currentNode = L.head
    while( currentNode != None):
...
'''
def deleteLinkedlist(L, element):
    position  = searchLinkedlist(L, element)
...
'''
def accessLinkedlist(L,position):
    retorno = None
...
'''
def updateLinkedlist(L,element,position):
    if position < (lengthLinkedList(L)):
        cont = 0
        currentNode = L.head
        while (currentNode != None):
            if cont == position :
