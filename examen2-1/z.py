...
"""
Recopilacion de funciones ya hechas
"""
from algo1 import*
from random import  *
from math import *
...
def sub(L,start,end):
    ...
    if start<=length(L):
        for i in range(0,start):
            currentNode=currentNode.nextNode
        add(N,currentNode.value)

    if end<=length(L):
        for j in range(start+1,end):
            currentNode=currentNode.nextNode
            insert(N,currentNode.value,length(N))

    return(N)


def concat(L,S):
    """retorna la union de las listas L y S"""
    N=LinkedList()
    currentNode=L.head
    while currentNode!=None:
